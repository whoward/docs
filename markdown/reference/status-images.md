...
3. Make sure to set a [build number](/reference/test-configuration/#recording-build-numbers) a [pass/fail](/reference/test-configuration/#recording-pass-fail-status) status and a [visibility](/reference/test-configuration/#job-visibility) (to either 'public', 'share' or 'public restricted') for every test that runs. You will be able to see that these are set correctly by seeing that your tests say "Pass" or "Failed" instead of "Finished" and that a build number is visible in the UI.
