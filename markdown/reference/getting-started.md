...
**Sauce Connect** is a secure tunneling app which allows you to execute tests securely when testing behind firewalls via an encrypted connection between Sauce Labs’ client cloud and your environment.
...
If auto-configuration fails, or the settings need to be overridden, there are a few command-line options that can be used to configure proxies manually: *-p, --proxy <host:port>, -w, --proxy-userpwd <user:pwd>, -T, --proxy-tunnel, and* ***--pac \<url>***
...
- Use WebDriver Expected Conditions – in order to manage latency, expected conditions will keep your tests from failing when page loads are long.
...
\- **Page Object Pattern** — When building your tests, you will want to make them as nimble to UI changes as possible. Setting them up using the page object design pattern will allow your tests to reference methods within the page object. When you make changes to your application under test you will only make necessary changes your page object rather than the multiple tests referencing the application. More information here:   http://docs.seleniumhq.org/docs/06_test_design_considerations.jsp#page-object-design-pattern
...
Use our Platform Configurator: https://docs.saucelabs.com/reference/platforms-configurator/#/, to quickly create your desired capabilities (with the configurations of your choice) that can be plugged into selenium script. 
...
### Parallel Execution
Another huge advantage of using Sauce labs is that you are able to test across multiple configurations in parallel significantly reducing execution time. Any language or test framework of choice allow for achieving concurrency either through multithreading or multiprocessing. 
...
###Test Assets
Configure the capture of test assets (screenshots, screencasts and videos) or whether you want to disable them completely: https://docs.saucelabs.com/reference/test-configuration/#optional-features

###REST API
Use the REST API to pull test assets and other job details: https://docs.saucelabs.com/reference/rest-api/

### Selenium - Specific
We use different versions of selenium versions for varying browser versions especially IE and firefox. You are able to change the version of selenium by specifying as a desired capability. https://docs.saucelabs.com/reference/test-configuration/#selenium-specific-modifications. It helps in debugging as well.
